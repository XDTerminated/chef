...
export default function ProfileSetupScreen() {
    ...
    const toggleTag = (tagId: string, type: "dietary" | "cuisine") => {
        if (type === "dietary") {
            setDietaryPreferences((prev) => prev.map((tag) => (tag.id === tagId ? { ...tag, selected: !tag.selected } : tag)));
        } else {
            setFavoriteCuisines((prev) => prev.map((tag) => (tag.id === tagId ? { ...tag, selected: !tag.selected } : tag)));
        }
    };
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            console.log(
                "Dietary Preferences:",
                dietaryPreferences.filter((tag) => tag.selected).map((tag) => tag.label)
            );
            console.log(
                "Favorite Cuisines:",
                favoriteCuisines.filter((tag) => tag.selected).map((tag) => tag.label)
            );
    ...
    const renderTag = (tag: Tag, type: "dietary" | "cuisine") => (
        <Pressable key={tag.id} style={[styles.tag, tag.selected && styles.tagSelected]} onPress={() => toggleTag(tag.id, type)}>
            <Text style={[styles.tagText, tag.selected && styles.tagTextSelected]}>{tag.label}</Text>
        </Pressable>
    );
    ...
    return (
        <ScrollView style={styles.container}>
            <View style={styles.content}>
                ...
                <Text style={styles.subtitle}>Help us personalize your cooking experience</Text>

                {/* Name Fields */}
                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>Personal Information</Text>
                    <View style={styles.nameRow}>
                        <TextInput style={[styles.input, styles.halfInput]} placeholder="First Name" value={firstName} onChangeText={setFirstName} placeholderTextColor="#999" />
                        <TextInput style={[styles.input, styles.halfInput]} placeholder="Last Name" value={lastName} onChangeText={setLastName} placeholderTextColor="#999" />
                    </View>
                </View>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.tagsContainer}>{dietaryPreferences.map((tag) => renderTag(tag, "dietary"))}</View>
                </View>

                {/* Favorite Cuisines */}
                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>Favorite Cuisines</Text>
                    <Text style={styles.sectionSubtitle}>Choose the cuisines you enjoy most</Text>
                    <View style={styles.tagsContainer}>{favoriteCuisines.map((tag) => renderTag(tag, "cuisine"))}</View>
                </View>

                {/* Complete Button */}
                <Pressable style={[styles.completeButton, isLoading && styles.completeButtonDisabled]} onPress={handleComplete} disabled={isLoading}>
                    {isLoading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.completeButtonText}>Complete Setup</Text>}
                </Pressable>
            </View>
        </ScrollView>
    );
