...
export const userAPI = {
    async createOrUpdateUser(userData: { clerkId: string; email: string; firstName?: string; lastName?: string; imageUrl?: string }) {
    ...
    async getUserByClerkId(clerkId: string) {
        ...
        try {
            ...
            const result = await response.json();
            console.log("User found via API:", result);
            return result.user || result; // Handle different response formats
        } catch (error) {
